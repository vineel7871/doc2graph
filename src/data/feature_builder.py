...
class FeatureBuilder():
    ...
    def __init__(self, d: int):
        ...
        if self.add_embs:
            ...
            self.text_embedder2 = fasttext.load_model(str(CHECKPOINTS / "cc.en.100.bin"))
        ...
        #         CHECKPOINTS / 'backbone_unet.pth')['weights'])
    ...
    def add_features(self, graphs: list, features: list) -> Tuple[list, int]:
        ...
        for id, g in enumerate(tqdm(graphs, desc='adding features')):
            ...
            if self.add_embs:
                ...
                [feats[idx].extend(self.text_embedder2.get_word_vector(features['texts'][id][idx])) for idx, _ in
                 enumerate(feats)]
                # chunks.append(len(self.text_embedder(features['texts'][id][0]).vector))
                if len(features['texts'][id]) > 0:
                    chunks.append(len(self.text_embedder2.get_word_vector(features['texts'][id][0])))
                else:
                    chunks.append(100)
