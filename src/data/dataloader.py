...
class Document2Graph(data.Dataset):
    ...
    def __init__(self, name: str, src_path: str, device=str, output_dir=str):
        ...
        # initialize class
        if not os.path.isdir(src_path): raise Exception(
        ...
        self.COLORS = {'invoice_info': (150, 75, 0), 'receiver': (0, 100, 0), 'other': (128, 128, 128),
                       'supplier': (255, 0, 255), 'positions': (255, 140, 0), 'total': (0, 255, 255)}

        # get graphs
        self.graphs, self.node_labels, self.edge_labels, self.paths = self.__docs2graphs()

        self.FB.text_embedder2 = None
        ...
        if self.node_labels:
            ...
            for idx, labels in enumerate(self.node_labels):
                self.graphs[idx].ndata['label'] = torch.tensor(
                    [np.where(target == self.node_unique_labels)[0][0] for target in labels], dtype=torch.int64)
        ...
        if self.edge_labels:
            ...
            for idx, labels in enumerate(self.edge_labels):
                self.graphs[idx].edata['label'] = torch.tensor(
                    [np.where(target == self.edge_unique_labels)[0][0] for target in labels], dtype=torch.int64)
    ...
    def print_graph(self, num=None, node_labels=None, labels_ids=None, name='doc_graph', bidirect=True, regions=[],
                    preds=None) -> Image:
